...
#else
            glFunctions = QOpenGLVersionFunctionsFactory::get<SGFGLFUNCTIONS>(context());
#endif
            if (glFunctions)
            {
                mGLVersionValid = true;
...
{
    std::sort(OpenGLWidget::sOverlayWidgets.begin(), OpenGLWidget::sOverlayWidgets.end(), [](OverlayWidget* a, OverlayWidget* b) -> bool { return a->getLayer() < b->getLayer(); });
...
OpenGLWidget* OpenGLWidget::getInstance()
{
    ...
    {
        qDebug() << "Window not properly instantiated!";
    }
