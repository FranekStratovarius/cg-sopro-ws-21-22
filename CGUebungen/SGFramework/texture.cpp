...
void Texture::convertImageToOpenGL(QImage *pImage)
{
#if QT_VERSION_MAJOR < 6
    *pImage = QGLWidget::convertToGLFormat(*pImage);
#else
...
void Texture::buildMaps(bool buildmipmaps)
{
    ...
    if (mTexDim != GL_TEXTURE_CUBE_MAP)
    {
        ...
        glFunctions->glTexImage2D(mTexDim, 0, GL_RGBA, mImage->width(), mImage->height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, mImage->bits());
    }
    else // GL_TEXTURE_CUBE_MAP
    {
        for (int i = 0; i < 6; i++)
        {
            ...
            glFunctions->glTexImage2D(faces[i], 0, GL_RGBA, mImages[i]->width(), mImages[i]->height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, mImages[i]->bits());
        }
    }
