...
void BulletPhysicEngine::copyMatrixToGeometry()
{
    ...
    for(int i=0;i<this->m_NumberOfThreads;i++)
    {
        ...
        lFuture.append(QtConcurrent::run(m_ThreadPool, m_MatrixCopyThreads.at(i), &BulletPhysicMatrixToGeometry::run));
#else
        lFuture.append(QtConcurrent::run(m_ThreadPool, &BulletPhysicMatrixToGeometry::run, m_MatrixCopyThreads.at(i)));
#endif
//        m_MatrixCopyThreads.at(i)->run();
    }
    for(int i=0;i<m_NumberOfThreads;i++)
